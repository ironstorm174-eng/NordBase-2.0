...
if (!code.includes("useTranslation")) {
  ...
  // Clean up existing useState imports to prevent duplicate
  code = code.replace("import { useState } from 'react';", "");
}

// Add hook
if (!code.includes("const { t } = useTranslation();")) {
  code = code.replace("export default function PartnerLandingPage({", "export default function PartnerLandingPage({\n  onBack\n}: PartnerLandingPageProps) {\n  const { t } = useTranslation();\n");
