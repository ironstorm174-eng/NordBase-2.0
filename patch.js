...
// 1. Update rpSlidesCount
code = code.replace(/const rpSlidesCount = \d+;/, 'const rpSlidesCount = 10;');

// 2. Replace navigation map for RP
const navOld = `                {[
...
// 4. Remove Algarve Case (which is currently rpSlide === 5 block, although above replace renamed it to rpSlide === 5? Wait, I replaced 6->5, 7->6, but not 5->... so now there are TWO rpSlide === 5).
// It's safer to just remove it using regex.
code = code.replace(/\/\* ========================================================= \*\/\s*\/\* SLIDE 06: REGIONAL CASE STUDY — ALGARVE \*\/[\s\S]*?(?=\/\* ========================================================= \*\/\s*\/\* SLIDE 06: BUSINESS)/, '');
